...
/**
 * SkewCards — animated skewed gradient showcase cards.
 *
 * NOTES ON INTEGRATION (see SETUP.md):
 *  - Requires a Tailwind color `dark` OR swap `bg-dark` for `bg-neutral-950`.
 *    Add to tailwind.config: theme.extend.colors.dark = "#0a0a0c".
 *  - The `blob` keyframes + `animation-delay-1000` utilities are provided
 *    inline via <style>. The original snippet had one malformed CSS rule
 *    (`.shadow-\[...`) which was removed — the arbitrary `shadow-[...]`
 *    Tailwind class already handles that box-shadow, so the rule was dead.
 *  - ⚠ Palette warning: these neon gradients (#ffbc00→#ff0058, #4dff03→#00d0ff)
 *    conflict with the RESPAWN brief (no acid/RGB). For a brand-consistent look,
 *    use <RespawnSpacesCards /> from "@/components/ui/respawn-spaces-cards".
 */
